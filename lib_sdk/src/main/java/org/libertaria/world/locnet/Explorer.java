...
public class Explorer implements Callable<List<NodeInfo>> {
    ...
    private static int DefaultClientPort = 16981;
    private static List<String> SeedServers = Collections.singletonList(
            //"ham4.fermat.cloud",
            //"ham5.fermat.cloud",
            //"ham6.fermat.cloud",
            //"ham7.fermat.cloud"
    ...
    private NodeInfo.ServiceType serviceType;
    private GpsLocation searchLocation;
    private float maxRadiusKm;
    private int maxNodeCount;


    private static org.gavaghan.geodesy.GeodeticCalculator geoCalc = new org.gavaghan.geodesy.GeodeticCalculator();
    private static org.gavaghan.geodesy.Ellipsoid reference = org.gavaghan.geodesy.Ellipsoid.WGS84;

    private double getDistanceKm(GpsLocation location) {
    ...
    private Session createSeedSession() throws IOException {
        ...
        while (!remainingSeeds.isEmpty()) {
            ...
            try {
                return new Session(seedHost, DefaultClientPort);
            } catch (Exception ex) {
    ...
    @Override
    public List<NodeInfo> call() throws IOException {
        ...
        while (oldClosestNode == null || // Just starting up or successfully got closer to the specified location
            ...
            try {
                session = new Session(newClosestNode);
            } catch (Exception ex) {
                break;
            }
        }
    ...
    private static <T> List<T> filter(Collection<T> items, IPredicate<T> predicate) {
    ...
    private List<NodeInfo> filterType(Collection<NodeInfo> nodes, final NodeInfo.ServiceType serviceType) {
        return filter(nodes, new IPredicate<NodeInfo>() {
            @Override
            public boolean appliesTo(NodeInfo node) {
    ...
    private List<NodeInfo> filterDistance(Collection<NodeInfo> nodes, final float maxDistanceKm) {
        return filter(nodes, new IPredicate<NodeInfo>() {
            @Override
            public boolean appliesTo(NodeInfo node) {
